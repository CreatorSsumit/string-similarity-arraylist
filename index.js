const { load } = require('spacy');


async function jaccardSimilarity(set1, set2) {


  const spacy = await load('en_core_web_sm'); // Load the English language model

  const doc1 = await spacy(set1);
  const doc2 = await spacy(set2);
